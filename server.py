...
def reqister():
    ...
    if form.validate_on_submit():
        ...
        post('http://0.0.0.0:5000/api/users', json={'name': form.name.data,
...
def add_news():
    form = PostForm()
    if form.validate_on_submit():
        post('http://0.0.0.0:5000/api/news', json={
...
def edit_news(news_id):
    form = PutForm()
    news = get(f'http://0.0.0.0:5000/api/news/{news_id}').json()['news']
    if form.validate_on_submit():
        put(f'http://0.0.0.0:5000/api/news/{news_id}', json={
            'text': form.text.data.capitalize()
        }).json()
        return redirect('/')
...
def news_delete(news_id):
    delete(f'http://0.0.0.0:5000/api/news/{news_id}').json()
    return redirect('/')
...
def like_news(news_id):
    ...
    if uid not in news['list_like'].split():
        ...
        edited.append(uid + '')
        edited = ' '.join(edited)
        put(f'http://0.0.0.0:5000/api/news/{news_id}', json={'like': news['like'] + 1,
                                                             'list_like': edited})
    else:
        edited = news['list_like'].split()
...
def dislike_news(news_id):
    ...
    if uid not in news['list_dislike'].split():
        ...
        edited.append(uid)
        edited = ' '.join(edited)
        put(f'http://0.0.0.0:5000/api/news/{news_id}', json={'dislike': news['dislike'] + 1,
                                                             'list_dislike': edited})
    else:
        edited = news['list_dislike'].split()
        del edited[edited.index(uid)]
